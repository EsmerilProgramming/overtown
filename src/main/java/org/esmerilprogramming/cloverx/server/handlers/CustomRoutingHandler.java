 ...
 */
public class CustomRoutingHandler extends RoutingHandler {


  // Matcher objects grouped by http methods.
  private final Map<HttpString, CustomPathTemplateMatcher<RoutingMatch>> matches = new CopyOnWriteMap<>();
  ...
  private final CustomPathTemplateMatcher<RoutingMatch> allMethodsMatcher = new CustomPathTemplateMatcher<>();
  ...
  public CustomRoutingHandler(boolean rewriteQueryParameters) {
    this.rewriteQueryParameters = rewriteQueryParameters;
  }

  public CustomRoutingHandler() {
  ...
  public void handleRequest(HttpServerExchange exchange) throws Exception {
    ...
    CustomPathTemplateMatcher.PathMatchResult<RoutingMatch> match = matcher.match(exchange.getRelativePath());
    if (match == null) {
      handleNoMatch(exchange);
      return;
    }
  ...
  public synchronized RoutingHandler addAll(CustomRoutingHandler routingHandler) {
    for (Map.Entry<HttpString, CustomPathTemplateMatcher<RoutingMatch>> entry : routingHandler.getMatches().entrySet()) {
      ...
      for (CustomPathTemplate template : entry.getValue().getPathTemplates()) {
  ...
  Map<HttpString, CustomPathTemplateMatcher<RoutingMatch>> getMatches() {
    return matches;
  }
